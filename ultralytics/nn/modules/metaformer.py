...
class MF_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (attn @ v).transpose(1, 2).contiguous().reshape(B, H, W, self.attention_dim)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class MetaFormerCGLUBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.res_scale2(x.permute(0, 3, 1, 2)).contiguous() + \
            self.layer_scale2(
                self.drop_path2(
                    self.mlp(self.norm2(x).permute(0, 3, 1, 2).contiguous())
                )
            )
        return x