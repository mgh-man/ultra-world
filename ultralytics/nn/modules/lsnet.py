...
try:
    ...
    # 使用新的 torch.amp API 并指定设备类型
    _custom_fwd = lambda func: custom_fwd(func, device_type='cuda')
    _custom_bwd = lambda func: custom_bwd(func, device_type='cuda')
except ImportError:
    # 向后兼容旧版本 PyTorch
    from torch.cuda.amp import custom_fwd, custom_bwd
    _custom_fwd = custom_fwd
    _custom_bwd = custom_bwd
...
try:
    ...
    class SkaFn(Function):
        ...
        def forward(ctx, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
            # 确保张量在 GPU 上
            if not x.is_cuda:
                # 如果在 CPU 上，回退到 CPU 实现
                return SkaFn._cpu_forward(ctx, x, w)
        ...
        @staticmethod
        def _cpu_forward(ctx, x: torch.Tensor, w: torch.Tensor) -> torch.Tensor:
            """CPU 后备实现"""
            ks = int(math.sqrt(w.shape[2]))
            pad = (ks - 1) // 2
            ctx.ks, ctx.pad = ks, pad
            ...
            # 使用 unfold 实现卷积
            x_unfold = torch.nn.functional.unfold(x, kernel_size=ks, padding=pad)
            x_unfold = x_unfold.view(n, ic, ks*ks, h, width)
            
            # 重塑 w 以匹配
            w_reshaped = w.view(n, wc, ks*ks, h, width)
            ...
            for i in range(ic):
                ...
                for j in range(ks*ks):
                    output[:, i] += x_unfold[:, i, j] * w_reshaped[:, wc_idx, j]
        ...
        def backward(ctx, go: torch.Tensor) -> tuple:
            if not go.is_cuda:
                # CPU 后备实现
                return SkaFn._cpu_backward(ctx, go)
        ...
        @staticmethod
        def _cpu_backward(ctx, go: torch.Tensor) -> tuple:
            """CPU 后备梯度计算"""
            x, w = ctx.saved_tensors
            # 简化的梯度计算
            return go, torch.zeros_like(w)
except:
    def _idx(i, n: int, c: int, h: int, w: int):
        pass
